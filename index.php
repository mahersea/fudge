                                ...
                                foreach (date_range($datestart,$dateend, "+1 day", "Y-m-d") as $dates) {
                                    ...
                                    foreach ($pdo->query($sql2) as $column) {                                   
                                        if($dates == $column['scheduledate']){
                                            ...
                                            //echo '<style>#draggable'.$row['id'].'-'.$column['scheduledate'].' { width: 50px; height: 50px; padding: 0em; background:#ffb477; float:left; border:solid; border-width:.5px; border-color:#000000 }</style>';
                                            echo '<script>$(function() {';
                                            echo '  $( "#draggable'.$row['id'].'-'.$column['scheduledate'].'" ).draggable({';
                                            echo '      containment:\'parent\',axis:\'x\',';
                                            echo '      grid: [51.5,51.5],';
                                            echo '      start: function(){';
                                            echo '        var offset = $(this).offset();';
                                            echo '        var xPos = offset.left;';
                                            echo '        $(\'#posX\').text(xPos);';
                                            echo '      },';
                                            echo '      stop: function(){';
                                            echo '        var offsetstop = $(this).offset();';
                                            echo '        var stopPos = offsetstop.left;';
                                            echo '        $(\'#posXstop\').text(stopPos);';
                                            echo '        var posX = document.getElementById("posX");'; 
                                            echo '        var startPos = posX.innerHTML;';
                                            echo '        var travel1 = parseInt(stopPos.valueOf(),10);';
                                            echo '        var travel2 = parseInt(startPos.valueOf(),10);';
                                            echo '        travel = travel1 - travel2;';
                                            echo '        $("#travel").text(travel);';
                                            // compute new date after drag
                                            echo '        dayChange = parseInt(travel/51);';
                                            //echo '        alert(dayChange);';
                                            echo '        var theDate = new Date("'.$column['scheduledate'].'");';
                                            echo '        theDate.setDate(theDate.getDate() + (dayChange + 1));';
                                            //echo '        alert(theDate);';

                                            echo '        var y = theDate.getFullYear(),';
                                            echo '            m = theDate.getMonth() + 1,';
                                            echo '            d = theDate.getDate();';
                                            echo '        var pad = function(val) { var str = val.toString(); return (str.length < 2) ? "0" + str : str};';
                                            echo '        dateString = [y, pad(m), pad(d)].join("-");';
                                            //echo '        alert(dateString);';
                                            echo '        ';
                                            echo '        ';
                                            echo '        ';
                                            echo '        ';

                                            echo '        reloadContent("'.$column['id'].'",dateString);';
                                                    //echo '        reloadContent("'.$column['id'].'","'.$column['scheduledate'].'");';
                                            echo '      }';
                                            echo '  });';
